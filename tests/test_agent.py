...
def _trunk_row(trunk_id, alias, recid, pdd=3000, attempts=10, duration=500, fails=1, answers=9):
    return {
        "field": [
            ...
            _field("alias", alias),
            _field("1h_pdd_ms", str(pdd)),
            _field("1h_call_attempt", str(attempts)),
            _field("1h_call_durationSec", str(duration)),
            _field("1h_call_fail", str(fails)),
            _field("1h_call_answer", str(answers)),
        ]
    }
